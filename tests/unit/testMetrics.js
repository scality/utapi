...
function testOps(schemaKey, keyIndex, metricindex, isNegativeValue, done) {
    ...
    if (keyIndex === 'storageUtilized' || keyIndex === 'numberOfObjects') {
        ...
        memBackend.zadd(key, timestamp, val, () => assertMetrics(schemaKey, schemaObject[schemaKey], props,
        ...
        memBackend.incrby(key, val, () => assertMetrics(schemaKey, schemaObject[schemaKey], props,
            isNegativeValue, done));
    } else {
        key = generateKey(schemaObject, keyIndex, timestamp);
        val = 1;
        ...
        memBackend.incr(key, () => assertMetrics(schemaKey, schemaObject[schemaKey], props,
            isNegativeValue, done));
    }
}
...
Object.keys(metricLevels).forEach(schemaKey => {
    ...
    describe(`Get ${metric} level metrics`, () => {
        ...
        it(`should return ${metric} level metrics for incoming bytes`, done => testOps(schemaKey, 'incomingBytes', 'incomingBytes', false, done));

        it(`should return ${metric} level metrics for outgoing bytes`, done => testOps(schemaKey, 'outgoingBytes', 'outgoingBytes', false, done));

        it(`should return ${metric} level metrics for delete bucket`, done => testOps(schemaKey, 'deleteBucket', 's3:DeleteBucket', false, done));

        it(`should return ${metric} level metrics for list bucket`, done => testOps(schemaKey, 'listBucket', 's3:ListBucket', false, done));

        it(`should return ${metric} level metrics for get bucket acl`, done => testOps(schemaKey, 'getBucketAcl', 's3:GetBucketAcl', false, done));

        it(`should return ${metric} level metrics for get bucket location`,
            done => testOps(schemaKey, 'getBucketLocation', 's3:GetBucketLocation',
                false, done));

        it(`should return ${metric} level metrics for put bucket acl`, done => testOps(schemaKey, 'putBucketAcl', 's3:PutBucketAcl', false, done));

        it(`should return ${metric} level metrics for get bucket cors`, done => testOps(schemaKey, 'getBucketCors', 's3:GetBucketCors', false,
            done));

        it(`should return ${metric} level metrics for put bucket cors`, done => testOps(schemaKey, 'putBucketCors', 's3:PutBucketCors', false,
            done));
        ...
        it(`should return ${metric} level metrics for put object`, done => testOps(schemaKey, 'putObject', 's3:PutObject', false, done));

        it(`should return ${metric} level metrics for copy object`, done => testOps(schemaKey, 'copyObject', 's3:CopyObject', false, done));

        it(`should return ${metric} level metrics for upload part`, done => testOps(schemaKey, 'uploadPart', 's3:UploadPart', false, done));

        it(`should return ${metric} level metrics for upload part copy`, done => testOps(schemaKey, 'uploadPartCopy', 's3:UploadPartCopy', false,
            done));
        ...
        it(`should return ${metric} level metrics for delete object`, done => testOps(schemaKey, 'deleteObject', 's3:DeleteObject', false, done));
        ...
        it(`should return ${metric} level metrics for get object`, done => testOps(schemaKey, 'getObject', 's3:GetObject', false, done));

        it(`should return ${metric} level metrics for get object acl`, done => testOps(schemaKey, 'getObjectAcl', 's3:GetObjectAcl', false, done));
        ...
        it(`should return ${metric} level metrics for put object acl`, done => testOps(schemaKey, 'putObjectAcl', 's3:PutObjectAcl', false, done));
        ...
        it(`should return ${metric} level metrics for head bucket`, done => testOps(schemaKey, 'headBucket', 's3:HeadBucket', false, done));

        it(`should return ${metric} level metrics for head object`, done => testOps(schemaKey, 'headObject', 's3:HeadObject', false, done));
