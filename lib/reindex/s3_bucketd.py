...
class updateRedis():
	...
	def read(self,ressource,name):
		...
		print ressource,
		print name,
		print r.get('s3:'+ressource+':'+name+':storageUtilized:counter'),
		print r.get('s3:'+ressource+':'+name+':numberOfObjects:counter'),
		print "zrange NB",
		print r.zrange('s3:'+ressource+':'+name+':numberOfObjects',-2,-1, withscores=True)[-1],
		print "zrange SU",
		print r.zrange('s3:'+ressource+':'+name+':storageUtilized',-2,-1, withscores=True)[-1]
	...
	def update(self,ressource,name, size, files):
		...
		numberOfObjectsCounter= 's3:%s:%s:numberOfObjects:counter' % ( ressource, name )
                storageUtilizedCounter = 's3:%s:%s:storageUtilized:counter' % ( ressource, name )
...
class S3BucketD(Thread):
	...
	def listbucket(self,session=None,marker = ""):
		...
		mark = urllib.quote_plus(m)
		params = "%s?listingType=DelimiterMaster&maxKeys=1000&marker=%s" % (self.bucket, mark)
