...
const { Clustering, errors, ipCheck, requestUrl } = require('@scality/arsenal');
const arsenalHttps = require('@scality/arsenal').https;
...
class UtapiServer {
     ...
     *
     * @param {string|object} component - request path (pathname plus query)
     * or parsed query object
    ...
    requestListener(req, res, router) {
        ...
        const { query: search, path, pathname } = requestUrl.parseRequestTarget(req.url);
