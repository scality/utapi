...
class UtapiReindex {
    ...
    _runScript(path) {
        ...
        process.stderr.on('data', data => {
            this._requestLogger.error('received error from job', {
                output: new Buffer(data).toString(),
            });
        });
        process.on('close', code => {
            if (code) {
                this._requestLogger.error(`job exited with code ${code}`);
            } else {
                this._requestLogger.info(`job exited with code ${code}`);
            }
        });
    }
